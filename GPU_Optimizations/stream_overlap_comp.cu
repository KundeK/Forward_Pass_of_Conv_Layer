...
__host__ void GPUInterface::conv_forward_gpu_prolog(const float *host_output, const float *host_input, const float *host_mask, float **device_output_ptr, float **device_input_ptr, float **device_mask_ptr, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    const int Height_out = Height - K + 1;
    const int Width_out = Width - K + 1;
    float* temp_host_output = (float*)host_output;
    int STREAM_X = (Batch*Channel*Height*Width)/10;
    int STREAM_Y = (Batch*Map_out*Height_out*Width_out)/10;
    int W_grid = (Width_out+TILE_WIDTH-1)/TILE_WIDTH;
    int H_grid = (Height_out+TILE_WIDTH-1)/TILE_WIDTH;
    int Y_grid = W_grid * H_grid;
    dim3 blockDim(TILE_WIDTH, TILE_WIDTH, 1);
    dim3 gridDim(Map_out, Y_grid, Batch/10);
    ...
    for(int i = 0; i < 10; i++){
        int x = STREAM_X*i;
        int y = STREAM_Y*i;
        cudaMemcpyAsync((*device_input_ptr)+x, host_input+x, STREAM_X*sizeof(float), cudaMemcpyHostToDevice, stream[i]);
        conv_forward_kernel<<<gridDim, blockDim, 0, stream[i]>>>((*device_output_ptr)+y, (*device_input_ptr)+x, *device_mask_ptr, Batch, Map_out, Channel, Height, Width, K);
        cudaMemcpyAsync(temp_host_output+y, (*device_output_ptr)+y, STREAM_Y*sizeof(float), cudaMemcpyDeviceToHost, stream[i]);
    }
    ...
    cudaStreamDestroy(stream[6]);
    cudaStreamDestroy(stream[7]);
    cudaStreamDestroy(stream[8]);
    cudaStreamDestroy(stream[9]);
