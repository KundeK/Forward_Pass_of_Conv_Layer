...
//This optimization is the same as the tiled_shared_mem.cu code, meant for implementing a tiled shared memory convolution. 
//Although the the streams with overlapping computation implementation "technically" has the lowest OP time, the results for 
//that are not as sure fire for this implementation. All other optimizations are in the "PM3_Optimizations" folder.
...
__host__ void GPUInterface::conv_forward_gpu_prolog(const float *host_output, const float *host_input, const float *host_mask, float **device_output_ptr, float **device_input_ptr, float **device_mask_ptr, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    // Pass double pointers for you to initialize the relevant device pointers,
